...
`wind_backend.py` reads the catalog to decide which file to serve, which is
what replaced the hardcoded filename it used to carry. Copernicus hands you
opaque hash filenames like `b2436709633b357f9ec6d77da9772ea0.nc`, so the
catalog exists to answer "what is actually in these things" once, cheaply, and
in a form both a human and the server can read.
...
# Default selection
# ---------------------------------------------------------------------------
def _choose_default(files, explicit):
    """Which file the backend serves unless told otherwise.

    Always paired with a written-out reason: a default that silently picks one
    of several plausible files is exactly the kind of thing you want to be
    able to read back later rather than re-derive.
    """
    if explicit:
        match = next((f for f in files if f["path"] == explicit), None)
        if match is None:
            raise SystemExit(
                f"--default {explicit!r} is not among the data files found: "
                f"{[f['path'] for f in files]}"
            )
        if not match["usableAsWindSource"]:
            raise SystemExit(
                f"--default {explicit!r} is not usable as a wind source: "
                f"{'; '.join(match['unusableReasons'])}"
            )
        return explicit, "set explicitly with --default"

    usable = [f for f in files if f["usableAsWindSource"]]
    if not usable:
        return None, "no usable wind source found"
    if len(usable) == 1:
        return usable[0]["path"], "the only usable wind source found"

    best = max(usable, key=lambda f: (f["time"]["count"], f["mtimeIso"], f["path"]))
    return best["path"], (
        f"most time steps ({best['time']['count']}) among "
        f"{len(usable)} usable wind sources"
    )
...
def _print_summary(catalog):
    ...
    for f in catalog["files"]:
        ...
        is_default = "  <- default" if f["path"] == catalog["default"] else ""
        print(f"{mark}{f['path']}  ({size_gb:.2f} GB){is_default}")
    ...
    print(f"\ndefault: {catalog['default']}  ({catalog['defaultReason']})")


# ---------------------------------------------------------------------------
# Entry point
# ---------------------------------------------------------------------------
def build_catalog(data_dir, stride, explicit_default):
    ...
    default, default_reason = _choose_default(files, explicit_default)
...
def main(argv=None):
    ...
    parser.add_argument("--default", dest="explicit_default", default=None,
                        help="pin the file the backend should serve, as a path relative to --data-dir")
    ...
    catalog = build_catalog(args.data_dir, args.stride, args.explicit_default)
    ...
    if catalog["default"] is None:
        print("WARNING: no usable wind source found -- the backend will serve synthetic wind.")
    return 0
