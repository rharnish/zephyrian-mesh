...
`sim-server` is just another *client* of
`weather-data-server/wind_backend.py`, the same way the browser used to be.
Python has since gained catalog-based file resolution and an additive `source`
field on the payload, but **the wire contract `sim-server` depends on is
unchanged** — see [Wind payload contract](#wind-payload-contract) below.
...
│  serves one ERA5 wind  │                                 │  owns the World:  │
│  snapshot as JSON      │                                 │  balloons, towers,│
└──────────────────────┘                                 │  wind, physics loop│
                                                            └────────┬─────────┘
                                                                     │ WS /ws
                                                                     │ JSON snapshots
...
1. **`weather-data-server/wind_backend.py`** parses an ERA5 NetCDF file and
   serves one pressure-level wind snapshot as JSON. Which file and which time
   step is resolved from `data/catalog.json` (generated by `catalog_data.py`),
   not hardcoded. Runs via `./run.sh`, or
   `uvicorn wind_backend:app --reload` in the `reginald` conda env as before —
   just from its renamed directory (was `WeatherData/`).
...
## Wind payload contract

`WindField` / `WindHeader` / `Level` in `src/wind_field.rs` deserialize
`wind_backend.py`'s `/api/wind-levels` response directly, and the same structs
re-serialize it for the browser. Two asymmetric rules follow from the serde
attributes:

- **Adding** top-level or per-level keys is safe — there is no
  `deny_unknown_fields`, so unknown keys are ignored. Python's `source` object
  (which file, which time step, whether the wind is synthetic) rides along this
  way.
- **Renaming or removing** any of `nx, ny, lo1, la1, lo2, la2, dx, dy,
  pressureHpa, altitudeM, u_data, v_data` breaks parsing — and there is no
  `serde(default)`, so it breaks *silently*: `fetch_wind_field` in `main.rs`
  falls back to `WindField::zero()` on a parse error with a single `warn!`. The
  sim then runs perfectly while balloons never move, which reads like a physics
  bug rather than a wire-format one.

`wind_field.rs`'s `deserializes_the_python_payload_shape` test parses a literal
copy of the payload to catch exactly that. Every other test there builds structs
in Rust and would not notice.

`sample()` also requires `levels` **sorted ascending by `altitudeM`** (Python
sorts them), and `sample_level` assumes `la1` is the maximum latitude with rows
descending on a uniform grid.
